...
function Skills($$payload) {
  ...
  $$payload.out += `<div class="relative flex flex-col min-h-screen min-w-screen" id="skills"><img src="/about/m5.png" class="absolute left-0 top-40 hidden sm:block w-auto max-h-[20vh] max-w-[20vw] object-contain transform scale-[0.85] sm:scale-90 md:scale-100" alt="decorative"> <img src="/about/m7.png" class="absolute right-0 bottom-20 hidden sm:block w-auto max-h-[20vh] max-w-[20vw] object-contain transform scale-[0.85] sm:scale-90 md:scale-100" alt="decorative"> <p class="absolute text-2xl sm:text-4xl md:text-5xl underline decoration-accent-red underline-offset-6 leading-[0.5] p-2 md:p-8">Skills &amp; Technologies</p> <div class="flex flex-col justify-center items-center gap-8 md:gap-12 grow p-4 md:p-8 pt-20 md:pt-24"><!--[-->`;
  for (let categoryIndex = 0, $$length = each_array.length; categoryIndex < $$length; categoryIndex++) {
    ...
    $$payload.out += `<div class="w-full max-w-5xl reveal"${attr_style(`--reveal-delay: ${stringify(categoryIndex * 120)}ms;`)}><h3 class="text-2xl md:text-3xl font-bold mb-4 md:mb-6 text-center">${escape_html(category.category)}</h3> <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6"><!--[-->`;
    for (let skillIndex = 0, $$length2 = each_array_1.length; skillIndex < $$length2; skillIndex++) {
      let skill = each_array_1[skillIndex];
      $$payload.out += `<div class="skill-card bg-accent-gray border border-border-gray rounded-lg p-4 md:p-6 reveal transition-all duration-300 hover:border-accent-red hover:scale-105 hover:shadow-lg hover:shadow-accent-red/20 svelte-1ymf1vv"${attr_style(`--reveal-delay: ${stringify(categoryIndex * 120 + skillIndex * 40)}ms;`)}><div class="flex justify-between items-center mb-3"><span class="text-lg md:text-xl font-semibold">${escape_html(skill.name)}</span> <span class="text-sm md:text-base text-gray-400">${escape_html(skill.level)}%</span></div> <div class="w-full bg-border-gray rounded-full h-2 overflow-hidden"><div class="skill-bar bg-gradient-to-r from-accent-red to-red-600 h-full rounded-full transition-all duration-1000 ease-out svelte-1ymf1vv"${attr_style(`width: ${stringify(skill.level)}%`)}></div></div></div>`;
    }
    $$payload.out += `<!--]--></div></div>`;
  }
  $$payload.out += `<!--]--></div></div>`;
...
function _page($$payload) {
  ...
  About($$payload);
  $$payload.out += `<!----></section> <section id="skills">`;
  Skills($$payload);
